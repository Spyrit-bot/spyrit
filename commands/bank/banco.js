...
let userdb = await userModel.findOneAndUpdate(query, update, options);
 ...
 let ano = +data.getFullYear()
    ...
    i.reply({
      content:user.id === i.author.id ? `O seu saldo no banco é de ${process.formatar(+userdb.economy.bank.toFixed(2))} e você está no top #${top} do ranking.` : `O saldo do banco de ${user} é ${process.formatar(+userdb.economy.bank.toFixed(2))} e ele está no top #${top} do ranking.`,
      allowMentions:{parse:[]}
    }).catch(()=>{})
    }
    else if(subcommand === "with"){
      let valor = args[1]
    if(!valor) return i.reply(`Use ${process.env.prefixo}bank with <valor ou all>.`).catch(()=>{});
    let usr = await userModel.findOne({ _id: i.author.id })
    ...
    return await i.reply({ content: `Você sacou com sucesso ${process.formatar(valor)} em sua conta bancária! Agora você tem ${process.formatar(usr.economy.bal)} no saldo e ${process.formatar(usr.economy.bank)} no banco!` }).catch(()=>{})
      ...
      let valor = args[1]
    if(!valor) return i.reply(`Use ${process.env.prefixo}bank dep <valor ou all>.`).catch(()=>{});
    let usr = await userModel.findOne({ _id: i.author.id })
    if(valor.toLowerCase() === "all") valor = usr.economy.bal
...
let usersdb = await userModel.find({ sim:true });
 ...
 function res(){
    ...
    if(reply){
      let col = reply.createMessageComponentCollector({
        filter: (e)=>e.user.id == i.author.id,
        time: 3_600_00
      })
      col.on("collect",col=>{
        let id = col.customId;
    ...
    else i.reply(`Comandos do banco:\n\n${process.env.prefixo}bank bal [@user] - Veja o saldo\n${process.env.prefixo}bank with <valor> - Saque seu saldo do banco\n${process.env.prefixo}bank dep <valor> - Deposite seu saldo no banco\n${process.env.prefixo}bank lb [pagina] - Veja os mais ricos`).catch(()=>{})
  },
