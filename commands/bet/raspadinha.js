...
export default {
  ...
  msgrun: async(bot,db,i,args)=>{
    const usrmodel = db.model("user")
    const transmodel = db.model("transactions")
    let usr = await usrmodel.findOne({ _id: i.author.id })
    ...
    if(r){
     let com = r.createMessageComponentCollector({
       filter:(e)=>{
         if(e.user.id != i.author.id) e.reply({ content: `Epa, você não é ${i.author}!`, ephemeral:true}).catch(()=>{})
         return e.user.id === i.author.id
         },
       time: 3_600_000
     })
     let end = false
     com.on("collect",async m=>{
       ...
       if(!fltr[0]){
         ...
         usr = await usrmodel.findOne({ _id: i.author.id })
         // Lateral
         if(res[0][0] === res[0][1] && res[0][0] === res[0][2]){
           winx[0] = true;
           winx[1] = true;
         ...
         let lc = bot.channels.cache.get(process.env.logs);if(lc){lc.send(`${i.author.globalName || i.author.username}(${i.author.tag}/${i.author.id}) apostou na raspadinha e ${win != 0 ? win > 0 ? `ganhou ${process.formatar(win-200)}` : `perdeu ${process.formatar(200-win)}` : `perdeu ${process.formatar(200)}`}.`).catch(()=>{}) }
    ...
    function random(){
      ...
      let {prefixo} = process.env
      i.reply(`Raspadinha:\n\n\`${prefixo}raspadinha comprar\` - compra um ticket na raspadinha\n\`${prefixo}raspadinha status\` - veja seu status da raspadinha\n\`${prefixo}raspadinha info\` - veja os ganhos da raspadinha`).catch(()=>{})
    }
  }
}